import tkinter as tk  #(standard Python interface to the Tk GUI toolkit) tk provides a library of basic elements of GUI widgets for building a graphical user interface 
from tkinter import ttk  # tkk provides access to the Tk themed widget set,
from PIL import Image, ImageTk  # Python Imaging Library.  PhotoImage class is used to display images. ImageTk module contains support to create and modify Tkinter BitmapImage and PhotoImage.
...
def submit_button(page, entry):
    submit_btn = tk.Button(page,text= "Submit", command=lambda:on_submit(page, entry), font="Raleway", bg="black", fg= "white", width= 10, height=1 )
    submit_btn.grid(row=1, column=0, padx=5, pady=5, sticky="n")       


# funtion to create label for an error message
...
# function to get answer given by the user
def on_submit(page, entry):
    global playername
...
def check_button(page, level, entry):
    check_btn = tk.Button(page,text= "Check answer", command=lambda:check_answer(page, level, entry), font="Raleway", bg="black", fg= "white", width= 15, height= 1 )
    check_btn.grid(row=2, column=0, padx=40, pady=10, sticky="e") 


# function to handle options 
def handle_option(page, choice):
    global level, clue, hint, answer_reveal
    ...
    if choice == "A":
        ...
        check_button(page, level, riddle_entry)
    elif choice == "B":
        if hint == 0:
...
def check_answer(page, level, riddle_entry):
    flag = False
    answer_dict = {'1-R1':'N2', '1-R2' : ['MOTHER', 'MUM', 'MOM', 'MUMMY', 'MAA'], '1-R3' :'SHADOW', '2-R1':'RED', '2-R2':'JAYDEN', '2-R3':'TON',
'3-R1':['MAID ELANOR', 'ELANOR', 'MAID'], '3-R2':'HEART', '3-R3': 'PENCIL' }
...
# funtion that gives answer directly
...
# Hint for first clue of level 1
def hint1a(page):
...
denary as well and then use the ASCII table to find corresponding characters.\n Now try answering the riddle.""", font=("Raleway", 10), fg="black")
    ...
    page.after(7000, hint_label.destroy)
...
def hint2b(page):
    ...
    page.after(7000, canvas.destroy)

    # hint_label = tk.Label(page, text="Your hint is: \n -Refer to a the above pattern and match the numbers to the alphabets on the keypad.", font=("Raleway", 10), fg="black")
    # hint_label.grid(row=2, column=0,pady=5)
    # page.after(7000, hint_label.destroy)
...
# funtion to add widgets to front page
...
# adding button
    ...
    # getstarted_btn.grid(column=0, row=0,pady=10)
...
- Keep in mind that you have a total of 3 hints which you can use at any point during the game.
...
# function to add widgets to first clue page
...
Background: “Rock a by baby, rock a bye”, the lullaby hummed by the doll which Mr. Paterson gifted Layla on her 3rd birthday echoed through the long hallway of Paterson Family's Grand Mansion. This voice was followed by a loud scream coming from Layla's room. It was 10:05 p.m when a black Mercedes abruptly stopped at the gate, and Mr. and Mrs. Paterson hurriedly entered 
the mansion in their formal suits, rushing upstairs where Layla's body was covered in white cloth. 5-year-old Layla was dead. Her body was reported by her nanny who went to the kitchen to grab snacks for her and returned to a horrifying sight of Layla covered in red with what seemed to be a pencil stabbed directly at her heart. To unravel the murder of Layla, your task is to complete the riddles.\n
Character Profile: 
- Mrs. Paterson: Mr. Paterson's second wife, who he married shortly after Layla's mother died. She was a renowned fashion designer, who wanted to have a baby. 
- Uncle Joseph: Layla's Uncle who had a huge fight with Mr. Paterson over their father's inheritance.  
- Aunty Teressa: A kind lady, who was disapproved of by Joseph's family because of her family background. 
- Nanny Angelina: She treated Layla like her own daughter. She was hired by Mr. Paterson's first wife due to her experience in handling babies. 
- Maid Elanor: She was known to be arrogant as she was favored by Mrs. Paterson and used to run errands for her. 
...
as she saw this as the perfect opportunity to get back at the Paterson's family. However, Oliver revealed that when he 
entered the room at 9:20 p.m. to give Layla her food as usual, Layla was already dead. Upon hearing this, Ray was 
thoroughly puzzled. However, he recollected something which could possibly be the answer to all this. To find out 
about the murderer, solve the following riddle:
...
# adding congratulatory message
    if level == 1 or level ==2:
        successmsg = tk.Label(congrats_page, text="CONGRATULATIONS,\n YOU HAVE SUCCESFULLY COMPLETED LEVEL "+ str(level), font=("Din Studio",25), bg="#D2B48C", fg="black")
...
# creating the final page containing truth for all levels
